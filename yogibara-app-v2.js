...
(() => {
  ...
  function installStorySheet(){
    ...
    sh.innerHTML=`<div class="ybStoryPanel"><div class="ybStoryHead"><h3>YOGIBARA 스토리 카드</h3><button id="ybStoryClose">×</button></div><div class="ybStoryPreview"><canvas id="ybStoryCanvas" width="1080" height="1920"></canvas></div><div class="ybStoryTools"><textarea id="ybStoryText" placeholder="스토리에 넣을 한 줄"></textarea><div class="ybPhotoPick"><label class="ybPhotoBtn">＋ 카드에 사진 추가<input id="ybStoryPhoto" type="file" accept="image/*"></label><img id="ybStoryPhotoPreview" class="ybThumb" alt="스토리 카드 사진"></div><div class="ybStoryActions"><button id="ybStoryShare">스토리로 공유</button><button id="ybStorySave">이미지 저장</button></div><div class="soft">공유 버튼을 누르면 휴대폰 공유창에서 Instagram 등 원하는 앱을 선택할 수 있어요.</div></div></div>`;
    ...
    document.getElementById('ybStoryShare').textContent='Instagram · 스토리 공유';
  ...
  async function openStory(entry){
    ...
    text.value=(entry.category==='share'||entry.subCategory==='share')?`${entry.title||'좋은 나눔'}로 마음을 나눴습니다.`:`${entry.title||'GOOD ACTION'}로 오늘의 좋은 행동을 기록했습니다.`;
  ...
  async function drawStory(){
    ...
    ctx.fillStyle='#6e635b';ctx.font='600 28px sans-serif';ctx.fillText((storyEntry.category==='share'||storyEntry.subCategory==='share')?'SHARE · GOOD ACTION':'TODAY · GOOD ACTION',82,165);
  ...
  window.YOGIBARA_MEDIA = window.YOGIBARA_MEDIA || {};
  window.YOGIBARA_MEDIA.openStory = openStory;
  ...
  function installCompleteStoryButton(){
    ...
    const obs=new MutationObserver(()=>{
      const entry=state.entries.slice().sort((a,b)=>(b.id||0)-(a.id||0))[0];
      b.style.display=sheet.classList.contains('open')&&entry?'block':'none';
    });
...
/* YOGIBARA IA v2 · ME / ACTION / SHARE / MAP / CONNECT */
(() => {
  'use strict';

  const DONATION_TOTAL = 25943645;

  function fmtWon(n){ return Number(n||0).toLocaleString('ko-KR') + '원'; }
  function dkey(){ return new Date().toISOString().slice(0,10); }
  function isMove(e){ return (e.category||'')==='move' || (e.subCategory||'')==='move' || e.type==='practice'; }

  function todayEntries(){
    const d=dkey();
    return (state.entries||[]).filter(e=>e.date===d).sort((a,b)=>(b.id||0)-(a.id||0));
  }

  function todayEnergy(){
    return todayEntries().reduce((sum,e)=>{
      try{return sum+entryEnergy(e);}catch(_){return sum+(Number(e.energy)||0);}
    },0);
  }

  function weekMoveCount(){
    const now=new Date();
    const day=(now.getDay()+6)%7;
    const monday=new Date(now); monday.setDate(now.getDate()-day); monday.setHours(0,0,0,0);
    return (state.entries||[]).filter(e=>{
      if(!isMove(e)||!e.date)return false;
  ...
  function injectIaStyles(){
    if(document.getElementById('ybIaStyles')) return;
    const s=document.createElement('style');
    s.id='ybIaStyles';
    s.textContent=`
      .top{justify-content:flex-start}.topBtns{display:none!important}.brand{cursor:pointer}
      .bottom{grid-template-columns:repeat(5,1fr)}
      .bottom .add{margin-top:0!important}
      .bottom .add i{width:auto!important;height:auto!important;border-radius:0!important;display:block!important;margin:0 0 2px!important;background:transparent!important;color:inherit!important;font-size:18px!important;box-shadow:none!important}
      .ybActionHead{display:flex;justify-content:space-between;align-items:end;padding:17px 2px 8px}
      .ybActionHead h2{margin:0;font:700 25px Georgia,"Noto Serif KR",serif}.ybActionHead span{font-size:9px;color:var(--muted)}
      .ybMeToday{margin-top:9px;padding:15px;background:linear-gradient(145deg,#eef0dc,#fff7ee)}
      .ybMeTodayTop{display:flex;justify-content:space-between;align-items:end;gap:10px}
      .ybMeToday strong{font-size:30px;color:#708033;line-height:1}.ybMeToday small{color:var(--muted);font-size:8px}
      .ybMeTodayMeta{display:grid;grid-template-columns:1fr 1fr;gap:1px;margin-top:12px;background:#ded5ca;border-radius:13px;overflow:hidden}
      .ybMeTodayMeta div{background:rgba(255,250,244,.96);padding:10px}.ybMeTodayMeta b{display:block;font-size:15px}.ybMeTodayMeta span{font-size:7px;color:var(--muted)}
      .ybInlineStoryBtn{margin-top:9px;border:1px solid var(--line);background:#f1e5da;color:var(--ink);border-radius:10px;padding:8px 10px;font-size:8px;font-weight:900}
      .ybDonationImpact{padding:17px;margin-bottom:10px;background:linear-gradient(145deg,#e8ecd8,#fff4ed)}
      .ybDonationImpact h3{margin:5px 0 3px;font-size:14px}.ybDonationAmount{font:700 33px Georgia,serif;color:#71813d;margin-top:4px}
      .ybDonationImpact p{margin:8px 0 0;font-size:10px;line-height:1.65;color:#6f655e}
      .ybShareBtns{display:grid;grid-template-columns:1fr 1fr;gap:7px;margin-top:12px}
      .ybShareBtns button{border:1px solid var(--line);background:#fff;border-radius:12px;padding:10px;font-size:9px;font-weight:900;color:var(--ink)}
      .ybShareBtns button:first-child{background:var(--olive);border-color:var(--olive);color:#fff}
      #me>.meEnergy,#me>.meStats{display:none!important}
      #home>.hero,#home>.flow,#home>.energyCard,#home>.network{display:none!important}
      #home .projectCard,#home .linkRow{display:none!important}
      #home>.sectionTitle[data-yb-hide="1"]{display:none!important}
      #market .screenHead h2{letter-spacing:-.02em}
    `;
    document.head.appendChild(s);
  }
  ...
  function restructureNav(){
    ...
    b.slice(0,5).forEach((btn,i)=>{
      const [dest,icon,label]=defs[i];
      btn.dataset.go=dest;
      btn.classList.remove('add','active');
      btn.innerHTML=`<i>${icon}</i>${label}`;
      btn.onclick=()=>go(dest);
    });
    b[0].classList.add('active');
  }

  function simplifyActionPage(){
    const home=document.getElementById('home');
    if(!home)return;
    let head=document.getElementById('ybActionHead');
    if(!head){
      head=document.createElement('div');
      head.id='ybActionHead';head.className='ybActionHead';
      head.innerHTML='<h2>ACTION</h2><span>오늘의 좋은 행동</span>';
      home.prepend(head);
    }

    // Keep only the action catalogue and its daily cap.
    [...home.children].forEach(el=>{
      const keep=el===head || el.id==='ybDailyCap' || el.id==='ybGoodActions';
      const isActionTitle=el.classList?.contains('sectionTitle') && (el.textContent||'').includes('오늘의 좋은 행동');
      if(keep||isActionTitle)return;
      el.style.display='none';
      if(el.classList?.contains('sectionTitle'))el.dataset.ybHide='1';
    });

    const title=[...home.querySelectorAll('.sectionTitle')].find(el=>(el.textContent||'').includes('오늘의 좋은 행동'));
    if(title){
      title.style.display='flex';
      title.innerHTML='<b>오늘의 ACTION</b><small>한 항목당 하루 1번 · 최대 20 ENERGY</small>';
    }
    const cap=document.getElementById('ybDailyCap');
    if(cap) cap.style.marginTop='0';
  }

  function buildMeToday(){
    const me=document.getElementById('me');
    const profile=me?.querySelector('.profile');
    if(!me||!profile)return;
    let card=document.getElementById('ybMeToday');
    if(!card){
      card=document.createElement('div');
      card.id='ybMeToday';card.className='ybMeToday card';
      profile.insertAdjacentElement('afterend',card);
    }
    const entries=todayEntries();
    card.innerHTML=`
      <div class="ybMeTodayTop"><div><small>오늘의 나</small><div style="margin-top:4px;font-size:14px;font-weight:900">${entries.length?`오늘 ${entries.length}개의 ACTION을 기록했어요.`:'오늘의 첫 ACTION을 기록해보세요.'}</div></div><div style="text-align:right"><small>오늘 ENERGY</small><strong>${Math.min(20,todayEnergy())}</strong></div></div>
      <div class="ybMeTodayMeta"><div><span>오늘 ACTION</span><b>${entries.length}개</b></div><div><span>이번 주 MOVE</span><b>${weekMoveCount()}회</b></div></div>
      ${entries.length?'':'<button id="ybStartAction" class="wideBtn primary" style="width:100%;margin-top:10px">ACTION 시작하기</button>'}`;
    const start=document.getElementById('ybStartAction');
    if(start)start.onclick=()=>go('home');
  }

  function enhanceMe(){
    const me=document.getElementById('me');
    if(!me)return;
    const h=me.querySelector('.screenHead h2'),sp=me.querySelector('.screenHead span');
    if(h)h.textContent='ME'; if(sp)sp.textContent='오늘의 기록';

    buildMeToday();

    const title=[...me.querySelectorAll('.sectionTitle')].find(x=>(x.textContent||'').includes('내 최근 기록')||(x.textContent||'').includes('오늘의 기록'));
    if(title)title.innerHTML='<b>오늘과 최근 기록</b><small>사진 · 한 줄 · 스토리</small>';

    const recent=(state.entries||[]).slice().sort((a,b)=>(b.id||0)-(a.id||0)).slice(0,6);
    const cards=[...document.querySelectorAll('#myRecent .recordCard')];
    cards.forEach((card,i)=>{
      const e=recent[i]; if(!e)return;
      let btn=card.querySelector('.ybInlineStoryBtn');
      if(!btn){
        btn=document.createElement('button');btn.className='ybInlineStoryBtn';btn.textContent='Instagram · 스토리 올리기';
        const left=card.firstElementChild||card; left.appendChild(btn);
      }
      btn.onclick=()=>window.YOGIBARA_MEDIA?.openStory?.(e);
    });
  }
  ...
  function buildShare(){
    ...
    if(!market)return;
    const h=market.querySelector('.screenHead h2'),sp=market.querySelector('.screenHead span');
    if(h)h.textContent='SHARE'; if(sp)sp.textContent='나눔 · 기부 · 중고판매';

    let impact=document.getElementById('ybDonationImpact');
    if(!impact){
      impact=document.createElement('div');
      impact.id='ybDonationImpact';impact.className='ybDonationImpact card';
      const tabs=document.getElementById('marketModeTabs');
      (tabs||market.firstElementChild)?.insertAdjacentElement('afterend',impact);
    }
    impact.innerHTML=`
      <div class="kicker">YOGIBARA GIVING</div>
      <h3>누적 기부금</h3>
      <div class="ybDonationAmount">${fmtWon(DONATION_TOTAL)}</div>
      <p>YOGIBARA를 통해 모인 나눔은 <b>중증 환아의 치료비 지원</b>에 사용됩니다. 작은 행동이 실제 치료로 이어집니다.</p>
      <div class="ybShareBtns"><button id="ybShareRecord">나눔 기록하기</button><button id="ybShareMarket">내 물건 올리기</button></div>`;
    document.getElementById('ybShareRecord').onclick=()=>{
      try{openCategory('share');}catch(_){document.getElementById('shareNowBtn')?.click();}
    };
    document.getElementById('ybShareMarket').onclick=()=>document.getElementById('marketAddBtn')?.click();

    const tabs=document.getElementById('marketModeTabs');
    if(tabs){
      const tb=[...tabs.querySelectorAll('button')];
      if(tb[0])tb[0].textContent='중고 나눔 · 판매';
      if(tb[1])tb[1].textContent='요가보따리 · 협업';
    }
    const hero=document.querySelector('#usedWrap .marketHero');
    if(hero){
      const kicker=hero.querySelector('.kicker'); if(kicker)kicker.textContent='SHARE MARKET';
      const hh=hero.querySelector('h3'); if(hh)hh.textContent='내 물건을 다시 쓰고, 나눔과 연결해요.';
      const soft=hero.querySelector('.soft'); if(soft)soft.textContent='일반 판매, 판매금 일부 기부, 전액 기부, 무료나눔을 선택할 수 있어요.';
    }
  }

  function hideTopDuplicates(){
    document.querySelector('.topBtns')?.setAttribute('style','display:none!important');
    const brand=document.querySelector('.brand');
    if(brand)brand.onclick=()=>go('me');
  }
  ...
  // First screen is ME. HOME is now the ACTION screen.
  try{go('me');}catch(_){
    document.querySelectorAll('.page').forEach(p=>p.classList.toggle('active',p.id==='me'));
  }
})();
